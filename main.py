...
commands = ('!loot', '!quests', '!search', '!value')
...
normalized_treasure = {key.lower(): value for key, value in treasure.items()}
# elif status == "Bounty":
...
print("Welcome to the Salty Fung's Wealth Abacus. For all intents and purposes")
print('it is assumed you are sailing as a Guild Emissary, and a list of available')
print('It is also assumed you are completing the highest rank voyage.')
print('For a complete list of commands, please type !help')
if gold_rush:
...
while is_running:
    if status.lower() in normalized_treasure:
        print(status, f"{normalized_treasure[status.lower()]} gold")
    elif status == "!help":
        print(commands)
    elif status == "!loot":
        ...
        if matches:
            print("\n".join(f"{item}: {treasure[item]}" for item in matches))
        else:
            print("No treasure matched that search.")
    elif status.startswith("!value "):
        items = [i.strip() for i in status[7:].split(",")]
        total = 0
        missing = []
        for item in items:
            if item.lower() in normalized_treasure:
                total += normalized_treasure[item.lower()]
            else:
                missing.append(item)
        print(f"Total value: {total} gold")
        if missing:
            print("These items weren't recognized:", ", ".join(missing))
    elif status == "X":
